...
void dialog_t::activate_dialog() {
  if (!active_dialogs.empty()) {
    if (this == active_dialogs.back()) return;
  ...
  if (shadow_window != nullptr) t3_win_set_depth(shadow_window, dialog_depth + 1);
  active_dialogs.push_back(this);
  if (active_popup != nullptr) active_popup->hide();
}
...
bool dialog_t::process_key(key_t key) {
  if (active_popup != nullptr) {
    if (active_popup->process_key(key)) return true;
  }
  ...
  if ((key & EKEY_META) || key == EKEY_F10) {
    for (widgets_t::iterator iter = widgets.begin(); iter != widgets.end(); iter++) {
      ...
      if ((*iter)->is_hotkey(key & ~EKEY_META)) {
        ...
        if ((*iter)->process_key(EKEY_HOTKEY)) return true;
...
bool dialog_t::is_child(window_component_t *widget) {
  if (active_popup != nullptr && active_popup->is_child(widget)) return true;
...
void dialog_t::set_active_popup(popup_t *popup) {
  if (popup == active_popup) return;
  popup_t *prev_active = active_popup;
  active_popup = popup;
  if (prev_active != nullptr) prev_active->dialog_base_t::hide();
}

void dialog_t::update_dialogs() {
  for (dialog_t *active_dialog : dialog_t::active_dialogs) active_dialog->update_contents();
  if (active_popup) active_popup->update_contents();
}
