...
void frame_t::set_child(widget_t *_child) {
  ...
  if (dimension & COVER_TOP) child_top = 0;
  if (dimension & COVER_LEFT) child_left = 0;
...
void frame_t::update_contents() {
  if (child != nullptr) child->update_contents();
  if (!redraw) return;
  redraw = false;
...
void frame_t::set_focus(focus_t focus) {
  if (child != nullptr) child->set_focus(focus);
}
...
bool frame_t::set_size(optint height, optint width) {
  ...
  if (child != nullptr) {
    ...
    if (!(dimension & COVER_TOP)) child_height--;
    if (!(dimension & COVER_BOTTOM)) child_height--;
    child_width = t3_win_get_width(window);
    if (!(dimension & COVER_LEFT)) child_width--;
    if (!(dimension & COVER_RIGHT)) child_width--;
...
void frame_t::set_enabled(bool enable) {
  if (child != nullptr) child->set_enabled(enable);
}
void frame_t::force_redraw() {
  if (child != nullptr) child->force_redraw();
}

void frame_t::set_child_focus(window_component_t *target) {
  container_t *container;
  if (target == child) child->set_focus(window_component_t::FOCUS_SET);
  container = dynamic_cast<container_t *>((widget_t *)child);
  if (container != nullptr) container->set_child_focus(target);
}
