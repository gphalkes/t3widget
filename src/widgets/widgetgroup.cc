...
bool widget_group_t::focus_next_int() {
  ...
  if (!impl->has_focus) return false;
...
void widget_group_t::focus_next() {
  if (!focus_next_int()) move_focus_down();
}

void widget_group_t::focus_previous() {
  if (!focus_previous_int()) move_focus_up();
}
...
void widget_group_t::add_child(widget_t *child) {
  ...
  if (impl->children.size() == 1) impl->current_child = 0;
}

widget_group_t::~widget_group_t() {
  for (widget_t *iter : impl->children) delete iter;
}

bool widget_group_t::process_key(key_t key) {
  if (impl->children.size() == 0) return false;

  if (impl->children[impl->current_child]->process_key(key)) return true;

  if (key == '\t') {
    if (focus_next_int()) return true;
  } else if (key == (EKEY_SHIFT | '\t')) {
    if (focus_previous_int()) return true;
  }
  return false;
}

void widget_group_t::update_contents() {
  for (widget_t *iter : impl->children) iter->update_contents();
}

void widget_group_t::set_focus(focus_t focus) {
  impl->has_focus = focus != window_component_t::FOCUS_OUT && impl->children.size() > 0;
  if (impl->children.size() == 0) return;
  switch (focus) {
...
bool widget_group_t::accepts_focus() {
  for (widget_t *iter : impl->children) {
    if (iter->accepts_focus()) return true;
  }
  return false;
}

void widget_group_t::force_redraw() {
  for (widget_t *iter : impl->children) iter->force_redraw();
}
...
bool widget_group_t::is_hotkey(key_t key) {
  ...
  for (widget_t *iter : impl->children) {
    if (!iter->is_enabled() || !iter->is_shown()) continue;

    if (iter->is_hotkey(key)) return true;
