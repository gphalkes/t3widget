...
expander_t::expander_t(const char *text) : impl(new implementation_t(text)) {
	...
	if ((impl->symbol_window = t3_win_new(window, 1, 2 + impl->label.get_width(), 0, 0, 0)) == nullptr)
		throw std::bad_alloc();
...
void expander_t::set_focus(focus_t _focus) {
	if (_focus == window_component_t::FOCUS_OUT) {
		...
		impl->focus = FOCUS_NONE;
			...
			(_focus == window_component_t::FOCUS_REVERT && impl->last_focus == FOCUS_SELF) || impl->child == nullptr ||
			!impl->is_expanded) {
		impl->focus = FOCUS_SELF;
	} else {
		impl->focus = FOCUS_CHILD;
...
widget_t *expander_t::is_child_hotkey(key_t key) {
	...
	if (!impl->is_expanded || impl->child == nullptr || !impl->child->is_shown() || !impl->child->is_enabled())
		return nullptr;
