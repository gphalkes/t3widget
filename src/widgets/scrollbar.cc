...
void scrollbar_t::update_contents() {
  ...
  if (impl->slider_size == 0) impl->slider_size = 1;
  ...
  if (impl->before >= impl->length - 2) impl->before = impl->length - 3;
  ...
  for (i = 1; i < impl->length - 1 && i < impl->before + 1; i++) {
    if (impl->vertical) t3_win_set_paint(window, i, 0);
  ...
  if (impl->vertical) t3_win_set_paint(window, impl->length - 1, 0);
...
void scrollbar_t::set_parameters(int _range, int _start, int _used) {
  if (impl->range == _range && impl->start == _start && impl->used == _used) return;
