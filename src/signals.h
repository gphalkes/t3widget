...
#warning BUILDING WITH EXPERIMENTAL C++11 SUPPORT
#include <functional>
#include <list>
#include <memory>
...
std::function<R(Args...)> mem_fun(typename internal::identity<C>::type *instance, R (C::*func)(Args...)) {
    return [=](Args... args){ return (instance->*func)(args...); };
}

template<typename R, typename C, typename... Args>
std::function<R(Args...)> mem_fun(typename internal::identity<C>::type *instance, R (C::*func)(Args...) const) {
    return [=](Args... args){ return (instance->*func)(args...); };
}
...
std::function<R(A, B)> bind(std::function<R(A, B, C, D)> f, typename internal::identity<C>::type c, typename internal::identity<D>::type d) {
...
class T3_WIDGET_API connection {
	public:
		...
		void disconnect() { if (func) func->disconnect(); }
		void block() { if (func) func->block(); }
		void unblock() { if (func) func->unblock(); }
	private:
...
class T3_WIDGET_API signal {
	public:
		...
		slot<R, Args...> make_slot() { return [this](Args... args) { return (*this)(args...); }; }
